...
class quizClass extends dbManagerClass {
    ...
    public function generateQuiz() {
        ...
        //$query_row = "SELECT * FROM test WHERE id=?";
    ...
    public function formatQuizPage() {
        ...
        if ($this->quizRow['question_difficulty'] == 'K') {
            ...
            $countdown_seconds = json_encode(180);
        }
        ...
        if ($this -> quizRow['question_type'] === 'select' || $this->quizRow['question_type'] === 'select-img') {
            ...
            for ($i = 1; $i <= 4; $i++)  {
                ...
                $idxOption = 'option'.$i;
                $idxOption_rand = 'option'.$rand_idx[$i - 1];
                $user_content .= self::createNewIRadio(array('display='.$this->quizRow[$idxOption_rand], 'name=user_answer', 'id='.$i, 'req=required', 'value='.$this->quizRow[$idxOption_rand], 'inside=div'));
            }
        }
