...
echo $test -> pageStart(
    $headpart = array('link_css=test.css'),
    $form = array('open', 'method=post', 'action=test2.php')
);
...
$p_inDiv_content = $test -> createNewTag(
    $set = array('type=p', 'value=newtest', 'inside=div', 'class=td1')
);
...
$csaba_text = $input_text_content = $test -> createNewTag(array('name=test_input', 'type=input-text', 'req=required', 'inside=p', 'value=', 'inside=p', 'inside-class=td3', 'placeholder=氏名を記入して下さい', 'style=background-color:white; color:black; width: 150px;'));


echo $checkbox = $test -> createNewCheckbox(array('display=apple', 'name=fruits[]', 'value=apple', 'inside=div', 'id=apple', 'req=required'));
echo $checkbox = $test -> createNewCheckbox(array('display=banana', 'value=banana', 'name=fruits[]', 'inside=div', 'id=banana', 'req=required'));
echo $checkbox = $test -> createNewCheckbox(array('display=pineapple', 'value=pineapple', 'name=fruits[]', 'inside=div', 'id=pineapple', 'req=required'));
